#!/usr/bin/env bash
# Run migration-raj.sql + test-migration.sql in ONE transaction against the
...
mig  = open("supabase/migration-raj.sql").read()
test = open("supabase/test-migration.sql").read()
sql  = "begin;\n" + mig + "\n" + test + "\nrollback;"
...
if isinstance(parsed, dict) and parsed.get("message"):
    ...
    sys.exit(1)
print("✓ migration + tests passed (rolled back, nothing persisted)")
